...
from models.ops.modules import MSDeformAttn, DecoderMSDeformAttn
...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        self.num_reppoints = nhead * dec_n_points
    ...
    def _reset_parameters(self):
        ...
        for m in self.modules():
            if isinstance(m, MSDeformAttn) or isinstance(m, DecoderMSDeformAttn):
                m._reset_parameters()
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        # cross attention
        self.cross_attn = DecoderMSDeformAttn(d_model, n_levels, n_heads, n_points)
        self.dropout1 = nn.Dropout(dropout)
        self.norm1 = nn.LayerNorm(d_model)
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index, valid_ratios, src_padding_mask=None):
        if self.use_ms_detr:
            # cross attention
            tgt2, sampling_locations = self.cross_attn(self.with_pos_embed(tgt, query_pos),
                                   reference_points, src, src_spatial_shapes, level_start_index, valid_ratios, src_padding_mask)
        ...
        return tgt_o2o, tgt_o2m, sampling_locations
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output, output_o2m, rep_point1 = layer(output, query_pos, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios, src_padding_mask, **kwargs)
            ...
            rep_points.append(rep_point1)
            
            # rep_points: (bs, num_queries, n_heads, 1, n_points, 2)
            # rep_box: (bs, num_queries, 4)
            rep_box = points2box(rep_point1)
            ...
            if self.bbox_embed is not None:
                ...
                new_reference_points = points2box(rep_point2)
