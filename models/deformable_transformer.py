...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, **kwargs):
        ...
        beta = 0.5
        bs, num_dec_q, _ = dec_query_o2o.shape
        for stage_idx, (enc_start_idx, enc_end_idx, dec_start_idx, dec_end_idx) in enumerate(combinations):
            if stage_idx != 0:


                # As sampling locations and attn weights are generated by decoder alone, without the need of encoder output,
                # we prefetch them to generate cross attention map.
                with torch.no_grad():
                    dec_sampling_locations, dec_attention_weights = self.decoder.prefetch_sampling_info(
                        dec_start_idx, dec_query_o2o, dec_ref, memory, spatial_shapes, level_start_index, valid_ratios, dec_query_pos, mask_flatten, **kwargs               
                    )
                    # use cls score to modulate attention weights
                    cls_score = self.decoder.class_embed[dec_start_idx - 1](dec_query_o2o).max(-1)[0].sigmoid()
                ...
                # encoder forward with selected queries
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def prefetch_sampling_info(self, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index, src_padding_mask=None):
        assert self.use_ms_detr
        dec_sampling_locations, dec_attention_weights = self.cross_attn.get_sampling_info(
            self.with_pos_embed(tgt, query_pos), reference_points, src, src_spatial_shapes, level_start_index, src_padding_mask
        )
        return dec_sampling_locations, dec_attention_weights
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def prefetch_sampling_info(self, layer_idx, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        sampling_location, attn_weight = layer.prefetch_sampling_info(
            output, query_pos, reference_points_input, src, src_spatial_shapes, src_level_start_index, src_padding_mask, **kwargs
        )
            
        return sampling_location, attn_weight
